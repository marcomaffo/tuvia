...
export default class Boss1 extends Enemy {
    ...
    if (!this.intro) {
      ...
      switch (this.currentAction) {
        ...
        case 0:
          ...
          break;
        case 1:
          if (this.timeSinceLastStateChange > 100) {
            this.currentState = (this.currentState + 1);
            this.timeSinceLastStateChange -= 100;
      ...
      switch (this.currentAction) {
          ...
          case 1:
              if (this.timeSinceLastStateChange > 100) {
                  this.currentState = (this.currentState + 1 * this.bossDirection);
                  this.timeSinceLastStateChange -= 100;
                  if (this.currentState < 0 || this.currentState > 8) {
                      this.currentAction += this.bossDirection;
                      this.currentState = 0;
                      this.timeSinceLastAction = 0;
                      this.starState = this.bossDirection * -1;
                  }
              }
              break;
          case 2:
  ...
  draw() {
    ...
    if (this.currentAction === 1 || this.currentAction === 3) {
      this.options.canvasContext.drawImage(this.bossTurningTexture, this.currentState * 196, 0, 196, 160, this.positionOnScreen.x, this.positionOnScreen.y, this.positionOnScreen.width, this.positionOnScreen.height);
    } else {
